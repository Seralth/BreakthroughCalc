...
def build_reference_pages(acc: dict, engine_data: dict,
    ...
    myrimon += (
        ...
        "cross the next BR requirement. Runs themselves carry across a "
        "realm ascension undiminished — near an ascension, hold unspent "
        "runs rather than spending them on a few extra of the current "
        "realm's fruit, and redeem them right after for the new realm's "
        "higher-tier fruit instead.</p>"
...
def build_guide_pages(acc: dict) -> list:
    ...
    voidbreak = (
        ...
        "<li><b>Save unspent Myrimon runs</b> rather than burning them on "
        "a few extra mortal-realm fruit — runs carry over the ascension "
        "undiminished (it's the banked fruit that decays, not the runs), "
        "so spend them right after ascending for Voidbreak-tier fruit "
        "instead.</li>"
